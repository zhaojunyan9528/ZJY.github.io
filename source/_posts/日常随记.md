...
**`?.`可选链操作符**
允许你读取位于连接对象链深处的属性的值，而不必显式地验证链中的每个引用是否有效。如果引用为空（null | undefined）,则返回undefined
...
按照特定次数重复执行固定代码块
与数组索引和数组长度结合起来遍历数组
可以用break,continue跳出或中断迭代，可以控制循环变量的更新，循环条件以及是否进行下一次迭代
...
`forEach`方法：
负责遍历可迭代对象。
不支持中断循环(return，break)，也不支持迭代过程添加删除数组元素
提供了回调函数和thisArg，该函数接收三个参数：当前值、当前索引和原数组本身。
通过使用 thisArg，你可以确保 this 在回调函数内部指向你期望的对象。如果不提供默认指向window
...
arr.forEach(function(element) {  
    this.increment(); // this 指向 obj  
    console.log(this.value); // 输出递增后的 value 值  
}, obj); // obj 作为 thisArg 传入
```

`for...in`方法：
遍历对象的可枚举属性（包含原型属性，不包含symbol）.如果你只想遍历对象自身的属性，可以使用 Object.keys(), Object.values(), 或 Object.entries() 方法结合 for...of 循环。
...
`for...of`：
遍历可迭代对象的值（例如数组，map，set，arguments，nodelist， TypedArray， string等）
支持使用break和continue中断
...
`map` 方法:
根据现有数组创建新数组，每个元素经过特定函数处理，返回结果是一个新数组。
map() 方法接受一个回调函数作为参数，该回调函数对数组的每个元素执行，并返回一个新的元素。此外，map() 还可以接受一个可选的 thisArg 参数，用于指定回调函数中 this 的值。
...
对象都有`__proto__`属性，指向他的构造函数的原型对象，原型对象也是原型，也有`__proto__`属性，指向它的构造函数的原型，这样一层一层形成的链式结构称为原型链，顶层为null。
...
在一些简单的异步操作中，Promise可能更加简洁和高效；而在需要处理复杂异步逻辑和错误处理的场景中，async/await可能更加适合。
...
+ 存储位置：cookie存储在浏览器中，sessionStorage/localStorage存储在浏览器内存中，若浏览器设置不保存数据，数据会丢失
...
解决：`<meta name="fomat-dection" content="telphone=no"`
...
android studio真机运行，排查后发现缺少高德地图的依赖库，导致页面白屏，下载最新依赖并引入后，仍然存在问题；
...
for循环是js提出就有的循环方法。forEach是ES5提出的，挂在可迭代对象原型上的方法。
forEach是一个迭代器，负责遍历可迭代对象。

es6引入了interable对象，可迭代对象，Array, Map, Set, String, arguments, NodeList都属于interable类型，他们特点就是都有[Symbol.iterator]方法，包含它的对象都被认为是可迭代的iterable.

**for和forEach的本质区别：**
for循环是一种循环机制，用来遍历出数组。forEach是一个迭代器，负责遍历可迭代对象。

可迭代对象调用内部的`[Symbol.iterator]`生成了迭代器对象，访问迭代器的next()方法来访问内部的值。每次调用next()方法，返回一个包含value和done属性的值，value为yield的返回值，done表示是否结束。这也是for...of的原理。
...
把调用next返回的值保存在item中过，直到value为undefined跳出循环。

**for和forEach的语法区别：**

+ forEach的参数：`arr.forEach((self, index, arr) => {}, this)`, this回调函数中this的指向
+ forEach的中断：for循环可以通过break, continue进行中断或跳出循环的操作，但是forEach不支持return continue break, 可以使用try/catch跳出循环
...
+ forEach删除自身原始，index不可以被重置，但是for循环删除自身元素可以重新设置index值
+ for循环可以控制循环起点，index = 1, forEach循环起点只能为0

**for和forEach的性能区别：**

+ 性能比较：for > forEach > map
+ 原因分析：for循环没有额外的函数调用栈和上下文，实现简单。forEach函数包含了参数和上下文，性能低于for。map会返回一个新的数组，数组的创建和赋值会分配内存空间，带来较大的性能开销。

...
解释原型链和继承的工作原理。
实现数组去重的方法有哪些？（可以结合题目提到的“unique”函数来展开）
描述一下闭包的概念及其应用场景。

**React / Vue / Angular：**

Vue2与Vue3之间的重要差异是什么？请介绍Vue3中Composition API的优势。
...
压缩资源和优化：
  ...
  css和js压缩去除空白字符和注释
  gzip压缩传输文本
  选用合适的图片格式，利用srcset图片响应式

懒加载和按需加载：

  首屏使用懒加载，元素出现在视口才进行加载
  使用异步加载js避免阻塞页面

预加载资源：
  
  使用preload预先加载并缓存当前页面重要内容

代码分割和动态导入：
  
  使用webpack进行代码分割成模块，实现首屏按需加载
  使用动态导入脚本，按需加载

优化渲染性能：  
  
  减少重排和重绘，合理操作dom
  使用css3硬件加速优化动画性能，比如transform will change requestAnimationFrame
